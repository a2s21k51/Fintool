...
export default function HomePage() {
  ...
  const [homeCycle, setHomeCycle] = useState<BillingCycle>('yearly');
  ...
  return (
    <div className="w-full min-h-screen bg-slate-50/50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12">
        ...
        {/* Subscription & Monetization Plans Section */}
        <section className="space-y-8 pt-4">
          <div className="text-center space-y-3 max-w-2xl mx-auto">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-50 dark:bg-amber-950/60 border border-amber-300 dark:border-amber-700/60 text-xs font-bold text-amber-800 dark:text-amber-300">
              <Sparkles className="w-3.5 h-3.5 fill-amber-400 text-amber-500" />
              <span>Subscription & Monetization Plans</span>
            </div>
            <h2 className="text-2xl sm:text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">
              Flexible Monthly & Yearly Plans
            </h2>
            <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400">
              Forever free for basic calculations. Upgrade to Pro for unlimited batch PDF compression, custom watermarking, and CA white-label exports.
            </p>

            {/* Monthly / Yearly Toggle on Home */}
            <div className="pt-2 flex items-center justify-center">
              <div className="inline-flex items-center p-1 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs">
                ...
                >
                  <span>Yearly</span>
                  <span className="px-1.5 py-0.5 rounded-full text-[9px] font-extrabold bg-amber-400 text-amber-950 uppercase tracking-tight">
                    Save 37%
                  </span>
                </button>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {SUBSCRIPTION_PLANS.map((plan) => {
              ...
              const price = homeCycle === 'yearly' ? plan.yearlyPriceINR : plan.monthlyPriceINR;
              const perMonth = homeCycle === 'yearly' && plan.yearlyPriceINR > 0 ? Math.round(plan.yearlyPriceINR / 12) : plan.monthlyPriceINR;
              ...
              return (
                <div
                  ...
                  className={`relative flex flex-col justify-between p-6 rounded-3xl border transition-all ${
                    isPro
                      ? 'bg-white dark:bg-slate-900 border-blue-600 dark:border-blue-500 shadow-xl shadow-blue-500/10 ring-2 ring-blue-600/20'
                      : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 shadow-sm'
                  }`}
                >
                  {plan.popular && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-[10px] font-extrabold uppercase tracking-wider shadow-sm">
                      Most Popular
                    </div>
                  )}
                    ...
                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                      {price === 0 ? (
                        <div className="text-2xl font-black text-slate-900 dark:text-white">
                          ₹0 <span className="text-xs font-semibold text-slate-400">/ forever free</span>
                        </div>
                      ) : (
                        <div>
                          <div className="flex items-baseline gap-1">
                            ...
                            <span className="text-xs font-semibold text-slate-500">
                              /{homeCycle === 'yearly' ? 'year' : 'month'}
                            </span>
                          </div>
                          {homeCycle === 'yearly' && (
                            <div className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400">
                              ₹{perMonth}/mo (Save 37% annually)
                            </div>
                          )}
                        </div>
                      )}
                    ...
                    <ul className="space-y-2 pt-2 text-xs">
                      {plan.features.slice(0, 4).map((f, i) => (
                        <li key={i} className="flex items-start gap-2">
                          ...
                          <span className={f.highlight ? 'font-bold text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400'}>
                            {f.title}
                          </span>
                        </li>
                      ))}
                  ...
                  <div className="pt-6 mt-6 border-t border-slate-100 dark:border-slate-800 space-y-3">
                    {plan.tier === 'free' ? (
                      <Link
                        href="/pricing"
                        className="w-full py-2.5 px-3 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-bold text-center transition-colors block"
                      >
                      ...
                      >
                        <span>{isCurrent ? 'Current Plan' : `Get ${plan.name}`}</span>
                    ...
                    {plan.tier !== 'free' ? (
                      <div className="pt-0.5 flex flex-col items-center justify-center gap-1 text-center">
                        <div className="flex items-center justify-center gap-1.5 flex-wrap">
                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 font-semibold text-[10px] border border-slate-200/60 dark:border-slate-700/60">
                            <QrCode className="w-2.5 h-2.5 text-emerald-600 dark:text-emerald-400" />
                            <span>UPI</span>
                          </span>
                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 font-semibold text-[10px] border border-slate-200/60 dark:border-slate-700/60">
                            <CreditCard className="w-2.5 h-2.5 text-blue-600 dark:text-blue-400" />
                            <span>Cards</span>
                          </span>
                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 font-semibold text-[10px] border border-slate-200/60 dark:border-slate-700/60">
          ...
          <div className="text-center pt-2">
            ...
            >
              <span>View Full Feature Comparison & FAQ Matrix</span>
              <ArrowRight className="w-3.5 h-3.5" />
            </Link>
          </div>
        </section>
